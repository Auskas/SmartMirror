...
class VoiceAssistant():
    ...
    def __init__(self):
        ...
        self.show_commands = ('показать', 'покажи', 'вывести', 'выведи', 'открой', 'открыть',
                              'включи', 'включить', 'show', 'open', ' on', 'watch')
    ...
    def cmd_handler(self, cmd):
        ...
        if (cmd.find('все виджеты') != -1 or cmd.find('all the widgets') != -1
            ...
            (self.cmd['clock'], self.cmd['spartak'], self.cmd['marquee'],
            self.cmd['stocks'], self.cmd['weather']) = True, True, True, True, True
        ...
        elif self.second_part_command(cmd) and (cmd.find('youtube') != -1 or
              cmd.find('видео') != -1 or cmd.find('video') != -1 or cmd.find('ютюб') != -1):
