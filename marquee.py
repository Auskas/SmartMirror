...
class Marquee(Canvas):
    ...
    def __init__(self, frame, newsrubot, voiceAssistant, gesturesAssistant, fps=125):
        ...
        self.text_id = self.create_text(0, 0, text=text, anchor="w", fill='lightblue', font=("SFUIText", 22, "bold"), tags=("text",))
        (x0, y0, x1, y1) = self.bbox("text")
        x1 = frame.winfo_screenwidth() - int(0.1 * (frame.winfo_screenwidth()))
        width = (x1 - x0)
        height = (y1 - y0)
        self.configure(width=width, height=height)
        self.place(relx=0.05, rely=0.82)
...
if __name__ == '__main__':
    ...
    newsruBot = NewsruBot()
    m = Marquee(window, newsruBot, voiceAssistant, gesturesAssistant)
    window.mainloop()
